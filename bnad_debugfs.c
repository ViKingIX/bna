...
{
	struct bnad_s *bnad = inode->i_private;
	...
	rc = bfa_ioc_debug_fwtrc(&bnad->bna.ioceth.ioc,
	...
	if (rc != BFA_STATUS_OK) {
		...
		kfree(fw_debug);
		fw_debug = NULL;
		pr_err("bnad %s: Failed to collect fwtrc\n",
...
bnad_debugfs_open_fwsave(struct inode *inode, struct file *file)
{
	...
	spin_lock_irqsave(&bnad->bna_lock, flags);
	rc = bfa_ioc_debug_fwsave(&bnad->bna.ioceth.ioc,
			fw_debug->debug_buffer,
			&fw_debug->buffer_len);
	spin_unlock_irqrestore(&bnad->bna_lock, flags);
	if (rc != BFA_STATUS_OK && rc != BFA_STATUS_ENOFSAVE) {
		...
		kfree(fw_debug);
		fw_debug = NULL;
		pr_err("bna %s: Failed to collect fwsave\n",
			pci_name(bnad->pcidev));
		return -ENOMEM;
	}
...
static int
bnad_get_debug_drvinfo(struct bnad_s *bnad, void *buffer, u32 len)
{
	...
	/* Get IOC info */
	spin_lock_irqsave(&bnad->bna_lock, flags);
	bfa_ioc_get_attr(&bnad->bna.ioceth.ioc, &drvinfo->ioc_attr);
	spin_unlock_irqrestore(&bnad->bna_lock, flags);
	...
	spin_lock_irqsave(&bnad->bna_lock, flags);
	ret = bfa_cee_get_attr(&bnad->bna.cee, &drvinfo->cee_attr,
	...
	init_completion(&fcomp.comp);
	spin_lock_irqsave(&bnad->bna_lock, flags);
	ret = bfa_flash_get_attr(&bnad->bna.flash, &drvinfo->flash_attr,
				bnad_cb_completion, &fcomp);
	if (ret != BFA_STATUS_OK) {
		spin_unlock_irqrestore(&bnad->bna_lock, flags);
		goto out;
...
bnad_debugfs_open_drvinfo(struct inode *inode, struct file *file)
{
	...
	if (rc != BFA_STATUS_OK) {
		...
		kfree(drv_info);
		drv_info = NULL;
		pr_err("bna %s: Failed to collect drvinfo\n",
			pci_name(bnad->pcidev));
		return -ENOMEM;
	}
			...
			size_t nbytes, loff_t *pos)
{
	...
	struct bnad_s *bnad = (struct bnad_s *)regrd_debug->i_private;
	ssize_t rc;
		...
		size_t nbytes, loff_t *ppos)
{
	struct bnad_debug_info *regrd_debug = file->private_data;
	struct bnad_s *bnad = (struct bnad_s *)regrd_debug->i_private;
	struct bfa_ioc_s *ioc = &bnad->bna.ioceth.ioc;
	int addr, len, rc, i;
	u32 *regbuf;
	...
	if (rc < 2) {
		pr_err("bna %s: Failed to read user buffer\n",
			pci_name(bnad->pcidev));
		kfree(kern_buf);
		return -EINVAL;
	}
	...
	rc = bfa_ioc_reg_offset_check(ioc, addr, len);
	if (rc) {
		pr_err("bna %s: Failed reg offset check\n",
			pci_name(bnad->pcidev));
		kfree(bnad->regdata);
		bnad->regdata = NULL;
		bnad->reglen = 0;
...
{
	struct bnad_debug_info *debug = file->private_data;
	struct bnad_s *bnad = (struct bnad_s *)debug->i_private;
	struct bfa_ioc_s *ioc = &bnad->bna.ioceth.ioc;
	int addr, val, rc;
	...
	/* offset and len sanity check */
	rc = bfa_ioc_reg_offset_check(ioc, addr, 1);
	if (rc) {
		pr_err("bna %s: Failed reg offset check\n",
			pci_name(bnad->pcidev));
		return -EINVAL;
...
void
bnad_debugfs_init(struct bnad_s *bnad)
{
	...
	if (!bna_debugfs_root) {
		...
		if (!bna_debugfs_root) {
			pr_err("BNA: debugfs root dir creation failed\n");
			return;
		}
	}
	...
	if (!bnad->port_debugfs_root) {
		...
		if (!bnad->port_debugfs_root) {
			pr_err("bna pci_dev %s: root dir creation failed\n",
				pci_name(bnad->pcidev));
			return;
		}
...
void
bnad_debugfs_uninit(struct bnad_s *bnad)
{
	int i;
