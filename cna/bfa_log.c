...
/**
 * @file bfa_log.c BFA log library
 */
...
struct bfa_log_info_s {
	uint32_t	start_idx;	/* !< start index for a module */
	uint32_t	total_count;	/* !< total count for a module */
	enum bfa_log_severity level;	/* !< global log level */
	bfa_log_cb_t	cbfn;		/* !< callback function */
};

static struct bfa_log_info_s bfa_log_info[BFA_LOG_MODULE_ID_MAX + 1];
static uint32_t bfa_log_msg_total_count;
 ...
 */
int
bfa_log_init(struct bfa_log_mod_s *log_mod, const char *instance_name,
			bfa_log_cb_t cbfn)
{
	struct bfa_log_msgdef_s *msg;
	uint32_t	pre_mod_id = 0;
	uint32_t	cur_mod_id = 0;
	uint32_t	i, pre_idx, idx, msg_id;
	...
	cbfn(log_mod, msg_id, NULL, "%s%s log: init OK, msg total count %u\n",
		BFA_LOG_CAT_NAME,
...
enum bfa_log_severity
bfa_log_get_msg_level(struct bfa_log_mod_s *log_mod, uint32_t msg_id)
{
	struct bfa_log_msgdef_s *msg;
	uint32_t	mod = BFA_LOG_GET_MOD_ID(msg_id);
	uint32_t	idx = BFA_LOG_GET_MSG_IDX(msg_id) - 1;
	...
	if (idx >= bfa_log_info[mod].total_count) {
		bfa_log_info[mod].cbfn(log_mod, msg_id, NULL,
 ...
 */
int
bfa_log(struct bfa_log_mod_s *log_mod, uint32_t msg_id, ...)
{
	va_list		ap;
	char		buf[256];
	struct bfa_log_msgdef_s *msg;
	...
	va_start(ap, msg_id);
	bfa_os_vsnprintf(buf, sizeof(buf), BFA_LOG_GET_MSG_FMT_STRING(msg), ap);

	if (log_mod) {
		log_mod->cbfn(log_mod, msg_id, ap, "%s[%s]%s%s %s: %s\n",
		...
		bfa_log_info[mod].cbfn(log_mod, msg_id, NULL, "%s%s%s %s: %s\n",
				BFA_LOG_CAT_NAME,
				bfa_log_severity[BFA_LOG_GET_SEVERITY(msg)],
				(msg->attributes & BFA_LOG_ATTR_AUDIT) ?
				" (audit) " : "", msg->msg_value, buf);
...
bfa_log_msgdef_t *
bfa_log_get_msg(struct bfa_log_mod_s *log_mod, uint32_t msg_id)
{
	struct bfa_log_msgdef_s	*msg;
	uint32_t		mod = BFA_LOG_GET_MOD_ID(msg_id);
	uint32_t		idx = BFA_LOG_GET_MSG_IDX(msg_id) - 1;
